...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если ПроверятьКодыМаркировки 
		И ИспользуютсяЧекопечатающиеУстройства Тогда
...
Процедура ЗаполнитьСтрокуЧекаМаркируемыеТовары(СтрокаПозицииЧека, РезультатРаспределения, ЭтоВерсия120)
	...
	// Частичное выбытие и РР - только в ФФД 1.2 и выше
...
Функция ЗаполнитьСписокОборудования(СообщениеОбОшибке = "")
	...
	// доступность проверяем только в режиме проверки марок "при сканировании"
	ТребуетсяПроверкаДоступностиККТ = ПроверятьКодыМаркировки
		И ИспользуетсяМаркируемаяПродукция(Истина)
		И ШтрихкодированиеОбщегоНазначенияИСМПКлиент.РежимПроверкиПриСканировании();
...
Процедура ПроверкаГотовностиККТЗавершение(РезультатПроверкиГотовности, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПроверкиГотовности.Результат Тогда
		...
		Если СтатусПоследнейСмены = Неопределено 
			...
			Если ДополнительныеПараметры.ЕстьМаркируемаяПродукцияВДокументе Тогда
			...
			Если ЗначениеЗаполнено(РезультатРаспределенияКодовМаркировки) И ПроверятьКодыМаркировки Тогда
...
Функция СписокТоваровОплатаСертификатом()
	...
	Для каждого СтрокаТаблицы Из ПараметрыПечатиЧека.ТаблицаДокумента Цикл
		...
		ТоварнаяПозиция.КодТовараТРУ         = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(СтрокаТаблицы.КодВидаТРУ, 30, "0","Справа");
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ПроверятьКодыМаркировки = Объект.ВидОперации = Перечисления.ВидыОперацийРозничнаяПродажа.Продажа // проверка КМ только при продаже
		И ИспользуютсяЧекопечатающиеУстройства
