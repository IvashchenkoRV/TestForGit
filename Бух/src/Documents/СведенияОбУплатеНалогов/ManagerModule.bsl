...
Процедура ЗагрузитьИзДанныхЛичногоКабинета(Организация, ТаблицаУплат, РезультатЗагрузки = Неопределено, ПерезаписыватьДокументы = Ложь) Экспорт
	...
	Для каждого СтрокаДанных Из ТаблицаУплат Цикл
		...
		Если ДатаСтроки <> ДатаДокумента Тогда
			ЗаписатьЗагружаемыйДокумент(ДокументОбъект, СлужебныйРезультат, СтрокаДанных.Сумма);
	...
	Если ПерезаписыватьДокументы Тогда
		...
		Для каждого СтрокаКУдалению Из СведенияКУдалению Цикл
			...
			ЗаписатьЗагружаемыйДокумент(УдаляемыйДокумент, СлужебныйРезультат, СтрокаДанных.Сумма, Истина);
		КонецЦикла;
	КонецЕсли;
