...
Процедура ОтправитьПлатежныйДокумент(НастройкаОбмена,
	...
	ПараметрыHTTPЗапроса.СертификатПодключения = СертификатПодключения;
...
Процедура ВыполнитьАутентификациюПоЛогину(Параметры, АдресРезультата) Экспорт
	...
	Если РезультатВыполнения.Статус Тогда
		...
		// Для создания HTTPS соединения с SSL нельзя повторно использовать один и тот же СертификатКлиентаФайл,
		// поэтому специально создаем новый СертификатКлиентаФайл
		СертификатПодключения = Новый СертификатКлиентаФайл(
			ПараметрыПодключения.ФайлСертификата,
			ПараметрыПодключения.СертификатПодключенияПароль);
...
Функция ОбновитьКлиентскийСекрет(ИдентификаторОрганизации,
	...
	СертификатПодключения = Новый СертификатКлиентаФайл(ФайлСертификата, ПарольСертификата);
...
Процедура ПолучитьСертификатыОрганизацииИДанныеПользователя(Параметры, АдресРезультата) Экспорт
	...
	Если ИнформацияОПользователе.ЭтоЕИО Тогда
		...
		ПараметрыПодключенияМетодЕИО = СлужебныеПараметрыПодключения(
			Параметры,
			"ПолучитьСертификатыОрганизацииИДанныеПользователя_ВсеСертификаты");
		СертификатПодключения = Новый СертификатКлиентаФайл(
			ПараметрыПодключенияМетодЕИО.ФайлСертификата,
			ПараметрыПодключенияМетодЕИО.СертификатПодключенияПароль);
		...
		УдалитьВременныйФайлСертификата(ПараметрыПодключенияМетодЕИО.ФайлСертификата);
		
	КонецЕсли;
	
	// Получим заново СертификатПодключения, т.к. для каждого нового соединения
	//  нужно использовать новый файл того же сертификата
	ПараметрыПодключенияМетодКрипто = СлужебныеПараметрыПодключения(
		Параметры,
		"ПолучитьСертификатыОрганизацииИДанныеПользователя_ЛичныеСертификаты");
	СертификатПодключения = Новый СертификатКлиентаФайл(
		ПараметрыПодключенияМетодКрипто.ФайлСертификата,
		ПараметрыПодключенияМетодКрипто.СертификатПодключенияПароль);
	...
	УдалитьВременныйФайлСертификата(ПараметрыПодключенияМетодКрипто.ФайлСертификата);
...
Функция ОтозватьТокенДоступа(Параметры) Экспорт
	...
	АдресРесурса = "ic/sso/api/v2/oauth/revoke?"
		+ "client_id=" + ПараметрыПодключения.ИдентификаторОрганизации
		+ "&client_secret=" + ПараметрыПодключения.КлиентскийСекрет
		+ "&token=" + ТокенДоступа
...
Функция ОтправитьHTTPЗапросРекурсивно(ПараметрыHTTPЗапроса, ИспользоватьЖурналирование)
	...
	КоличесствоПопыток = 0;
	
	Для Каждого ИмяЦепочкиСертификатов Из ИменаЦепочекСертификатов Цикл
		
		Соединение = СоединениеССервером(
			АдресСервера, , ПараметрыHTTPЗапроса.СертификатПодключения, ИмяЦепочкиСертификатов);
		КоличесствоПопыток = КоличесствоПопыток + 1;
		...
		Исключение
			...
			Если КоличесствоПопыток = ИменаЦепочекСертификатов.Количество() Тогда
				...
				ТекстЗаголовка = СтрШаблон(ТекстЗаголовка, ПараметрыHTTPЗапроса.АдресРесурса, КоличесствоПопыток, ИспользуемыйСертификат);
...
Процедура ПолучитьСтраницуВыписки(Параметры,
	...
	СертификатПодключения = Новый СертификатКлиентаФайл(
		ПараметрыЗапроса.ФайлСертификата, ПараметрыЗапроса.СертификатПодключенияПароль);
