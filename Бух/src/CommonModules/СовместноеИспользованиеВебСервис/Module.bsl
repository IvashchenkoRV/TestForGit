...
Функция СоздатьОрганизацию(ДанныеОрганизации) Экспорт
	...
	Если Справочники.Организации.КоличествоОрганизаций() > 0
		И ОбщегоНазначенияБП.ЭтоПростойИнтерфейс() Тогда
		РезультатСоздания.Ошибка = Истина;
		РезультатСоздания.СообщениеОбОшибке = НСтр("ru = 'В информационной базе включен простой интерфейс. В этом режиме поддерживается только одна организация.
		|Поэтому создание еще одной недоступно, т.к. организация уже создана'");
		Возврат РезультатСоздания;
	КонецЕсли;
