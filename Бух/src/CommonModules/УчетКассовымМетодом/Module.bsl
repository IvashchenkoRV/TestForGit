...
Функция УчетБезЗакрывающихДокументовВозможен(Организация, Дата) Экспорт
	...
	ПлательщикНДСУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, Дата);
	
	УСНБезЗакрывающих =
		СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная И ПрименяетсяУСНДоходы И Не ПлательщикНДСУСН;
