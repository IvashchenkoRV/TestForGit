...
Функция ДополнительныеСведенияУнифицированнойФормыТ2(СтрокиДанных, ДатаОтчета, КлючВарианта) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		ПараметрыВременнойТаблицы.ИмяВТСотрудникиПериоды = "ВТКадроваяИсторияСотрудников";
		ОписанияНачислений = КадровыйУчет.ОписанияСоставаНачисленийПоВременнойТаблице(Запрос.МенеджерВременныхТаблиц, ПараметрыВременнойТаблицы);
...
Функция ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ПараметрыВременнойТаблицы, ИспользоватьКраткиеПредставления, ВыводитьДенежныеПоказателиСВалютой, ФормироватьСПериодичностьДень) Экспорт
	
	Перем ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник;
	
	ПараметрыВременнойТаблицы.Свойство("ИмяВТСотрудникиПериоды", ИмяВТСотрудникиПериоды);
	ПараметрыВременнойТаблицы.Свойство("ИмяПоляПериод", ИмяПоляПериод);
	ПараметрыВременнойТаблицы.Свойство("ИмяПоляСотрудник", ИмяПоляСотрудник);
