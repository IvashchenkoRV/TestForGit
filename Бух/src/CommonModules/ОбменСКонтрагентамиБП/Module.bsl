...
Функция СоставительДокумента(СведенияОбОрганизации) 
	...
	Если СведенияОбОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		Постфикс = СтрШаблон(НСтр("ru = ', ИНН/КПП %1/%2'"), СведенияОбОрганизации.ИНН, СведенияОбОрганизации.КПП);
	Иначе
...
Процедура ЗаполнитьДанныеДляСЧФДОПИнформацияПродавцаФНС_5_02(Основание, ДанныеДляФормированияЭД, ДеревоДанных, Описание, Отказ)
	...
	Если Не (ДанныеДляФормированияЭД.ЭтоСчетФактураНаАванс
			ИЛИ ДанныеДляФормированияЭД.ЭтоКомиссионнаяТорговля
			ИЛИ Не ДанныеДляФормированияЭД.ОснованиеСФУчаствуетВЭДО) Тогда
		ЗаполнитьРасширеннуюЧастьУПД_5_02(ДанныеДляФормированияЭД, ДеревоДанных);
	КонецЕсли;
...
Процедура ЗаполнитьРасширеннуюЧастьУПД_5_02(ДанныеДляФормированияЭД, ДеревоДанных)
...
Функция ДанныеДокументаИСчетаФактурыУКД(Знач СсылкаНаОбъект)
	...
	Если ЗначениеЗаполнено(Данные.Документ) Тогда
		...
		КонецЕсли;
		Если ТипЗнч(Данные.Документ) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") И СписокРеквизитов.Найти("Контрагент") = Неопределено Тогда
			СписокРеквизитов.Добавить("Контрагент");
		КонецЕсли;
		...
		Данные.ЕстьПрослеживаемыеТовары = Не РеквизитыДокумента.СведенияПрослеживаемости.Пустой();
...
Процедура ЗаполнитьДокументПоступленияТоваровУслуг(Документ, ДанныеДокумента, СпособОбработки = Неопределено, ОписаниеОшибки = "")
	...
	Если СпособОбработкиПервичногоДокумента = СпособыОбработкиВходящихЭД.ПоступлениеМатериаловВПереработку
...
Функция ДанныеУчастникаЭД(ПараметрыУчастника)
	...
	ЭтоЭлектронноеАктированиеЕИС =
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОбУчастнике, "ЭтоЭлектронноеАктированиеЕИС", Ложь);
	Если ЭтоЭлектронноеАктированиеЕИС Тогда
	...
	Если ДанныеСтраны.Страна <> Справочники.СтраныМира.Россия Тогда
		...
		ТипУчастникаИЛ.Вставить("ИныеСведения", ?(ЗначениеЗаполнено(СведенияОбУчастнике.НалоговыйНомерВСтранеРегистрации),
	...
	Иначе
		...
		Данные.ТипУчастника[ТипУчастника].Вставить("СвидетельствоОГосРегистрации",
			БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(СведенияОбУчастнике));
	КонецЕсли;
	...
	Если СведенияОбУчастнике.Свойство("НомерСчета", НомерСчета) И ЗначениеЗаполнено(НомерСчета) 
			И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОбУчастнике, "ЦифровойСчет", Ложь) Тогда
		БанковскиеРеквизиты = Данные.БанковскиеРеквизиты;
