...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	КППДоЗаписи = ТекущийОбъект.ДополнительныеСвойства.Свойство("КППДоЗаписи");
	Если ЗначениеЗаполнено(КППДоЗаписи)
		И ЗначениеЗаполнено(ТекущийОбъект.КПП)
		И КППДоЗаписи <> ТекущийОбъект.КПП Тогда
		СервисСверкиРасчетов.ПриСменеКППУчастникаБизнесСети(ТекущийОбъект.Ссылка);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.ПростойИнтерфейс Тогда
		ПоказатьПомощникВнесенияИзмененийЕГР = ЗначениеЗаполнено(Объект.Ссылка)
			...
			И Не ЭтоСамозанятый
			И (ЗначениеЗаполнено(Объект.ИНН) Или ЗначениеЗаполнено(Форма.ЕГРЗаявление));
	Иначе
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ЕГРЗаявление) Тогда
		Элементы.ЕГРСформироватьПакетДокументов.Заголовок = НСтр("ru = 'Показать пакет документов'");
	Иначе
...
Процедура УстановитьСостояниеЗаявлениеОВнесенииИзмененийВЕГРН()
	...
	Если ЗначениеЗаполнено(ЕГРЗаявление) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеЗаявления) Тогда
			...
			ДанныеПомощникаЗаполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ДанныеУведомления,
...
&НаКлиенте
Процедура ЗакрытьФормуНалогиИОтчеты()
	
	Оповестить("ЗакрытьФормуНалогиИОтчеты", ЭтотОбъект.УникальныйИдентификатор);
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.ДекорацияЭмблемаДиректБанк);
	Элементы.ГруппаРекламаДиректБанк.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
