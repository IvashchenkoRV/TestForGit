...
Функция КодОшибкиПроверкиНомераКонтракта(Знач ДатаПостановкиКонтрактаНаУчет, Знач УникальныйНомер)
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ДатаПостановкиКонтрактаНаУчет)
		...
		Возврат 2;
	Иначе
		ГодУНВК = Строка(Год(ДатаПостановкиКонтрактаНаУчет) - 2000);
		МесяцУНВК = Строка(Месяц(ДатаПостановкиКонтрактаНаУчет));
		Если СтрДлина(МесяцУНВК) = 1 Тогда
			МесяцУНВК = "0" + МесяцУНВК;
		КонецЕсли;
		
		Если ДатаПостановкиКонтрактаНаУчет >= '20240401'Тогда
			Если СтрПодобнаПоРегулярномуВыражению(УникальныйНомер,
				ГодУНВК + МесяцУНВК + "[0-9]{4}/[0-9]{4}/GU[0-9]{2}/[0-9]{1}/[0-9]{1}") Тогда
			// Уникальный номер валютного контракта соответствует шаблону после 01.04.2024 года
			// ГГММХХХХ/ХХХХ/GUХХ/Х/Х, где X - число от 0 до 9, где ГГ - последние 2 цифры года и
			// ММ (числа от 01 до 12)- месяц, в котором контракту присвоен уникальный номер,
			// GU - символы "G" и "U".
				Возврат 0;
			Иначе
				Возврат 3;
			КонецЕсли;
		ИначеЕсли СтрПодобнаПоРегулярномуВыражению(УникальныйНомер,
				ГодУНВК + МесяцУНВК + "[0-9]{4}/[0-9]{4}/[0-9]{4}/[0-9]{1}/[0-9]{1}") Тогда
			// Уникальный номер валютного контракта соответствует шаблону до 01.04.2024 года
			// ГГММХХХХ/ХХХХ/ХХХХ/Х/Х, где X - число от 0 до 9, где ГГ - последние 2 цифры года и
			// ММ (числа от 01 до 12)- месяц, в котором контракту присвоен уникальный номер.
			Возврат 0;
		Иначе
			Возврат 4;
...
Функция ПояснениеКОшибкеВНомереУНК(КодОшибкиПроверкиКонтракта)
	...
	ИначеЕсли КодОшибкиПроверкиКонтракта = 3 Тогда
		Возврат НСтр("ru='Уникальный номер валютного контракта не соответствует шаблону:
			|""ГГММХХХХ/ХХХХ/GUХХ/Х/Х"", где X - число от 0 до 9, ""GU"" - символы,
			|ГГ - последние 2 цифры года и ММ - числа месяца (от 01 до 12),
			|в котором контракту присвоен уникальный номер'");
	ИначеЕсли КодОшибкиПроверкиКонтракта = 4 Тогда
		Возврат НСтр("ru='Уникальный номер валютного контракта не соответствует шаблону:
			|""ГГММХХХХ/ХХХХ/ХХХХ/Х/Х"", где X - число от 0 до 9
			|ГГ - последние 2 цифры года и ММ - числа месяца (от 01 до 12),
			|в котором контракту присвоен уникальный номер'");
	КонецЕсли;
