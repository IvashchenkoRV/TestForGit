...
Процедура ЗаполнитьТаблицуПоиска_НастройкиУчета(ТаблицаПоиска)
	...
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.Справочники.Организации.Команды.ПростойИнтерфейс_УчетнаяПолитика, Организация),
		"ПанельНастройкиНастройкиУчета", "УчетнаяПолитика",
		НСтр("ru='Учетная политика'"), НСтр("ru='Учетная политика'"));
	
	ПредставлениеСсылки = НСтр("ru='Начальные остатки'");
	ПараметрыФормы = Новый Структура("Организация", Организация);
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.Обработки.ВводНачальныхОстатков, ПредставлениеСсылки, ПараметрыФормы),
		"ПанельНастройкиНастройкиУчета", "НачальныеОстатки",
		ПредставлениеСсылки, НСтр("ru='Помощник ввода начальных остатков'"));
	
	ПредставлениеСсылки = НСтр("ru='Уставный капитал'");
	Отбор = Новый Структура("Организация", Организация);
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.Документы.ФормированиеУставногоКапитала, ПредставлениеСсылки, ПараметрыФормы),
		"ПанельНастройкиНастройкиУчета", "УставныйКапитал",
		ПредставлениеСсылки, НСтр("ru='Уставный капитал'"));
	
	ПредставлениеСсылки = НСтр("ru='Подразделения'");
	Отбор = Новый Структура("Владелец", Организация);
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.Справочники.ПодразделенияОрганизаций, ПредставлениеСсылки, ПараметрыФормы),
		"ПанельНастройкиНастройкиУчета", "Подразделения",
		ПредставлениеСсылки, НСтр("ru='Подразделения'"));
	
	ПредставлениеСсылки = НСтр("ru='Регистрации в налоговых органах'");
	Отбор = Новый Структура("Владелец", Организация);
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.Справочники.РегистрацииВНалоговомОргане, ПредставлениеСсылки, ПараметрыФормы),
	...
	ПредставлениеСсылки = НСтр("ru='Лимиты остатка кассы'");
	Отбор = Новый Структура("Организация", Организация);
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	ДобавитьСтрокуВТаблицуПоиска(ТаблицаПоиска,
		ПолучитьНавигационнуюСсылку(Метаданные.РегистрыСведений.ЛимитОстаткаКассыОрганизаций, ПредставлениеСсылки, ПараметрыФормы),
