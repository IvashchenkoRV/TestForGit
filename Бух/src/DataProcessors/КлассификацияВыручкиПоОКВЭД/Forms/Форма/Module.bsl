...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Объект.ДатаНачала = НачалоГода(ДобавитьМесяц(ТекущаяДатаСеанса(), -12));
...
Процедура ЗагрузитьРезультат(АдресРезультата, ДополнительныеПараметры)
	...
	ОКВЭДОрганизацииПредставление = Результат.ОКВЭДОрганизации.Представление;
	...
	Для Каждого СтрокаГруппировки Из СтрокаРодитель.Строки Цикл
		...
		Если СтрНайти(ОКВЭД, ";") = 0 И ЗначениеЗаполнено(ОКВЭД) Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				СтрокаРаспределения.ОКВЭДНаименование = 
					Прав(СтрокаГруппировки.ОКВЭД, СтрДлина(СтрокаГруппировки.ОКВЭД) - СтрДлина(СтрокаГруппировки.ОКВЭДКод) - 1);
