...
Функция ВыгрузитьДокумент(Документ) Экспорт
	...
	ДанныеДляЭкспортаПлатежногоПоручения = Обработки.КлиентБанк.ДанныеДляЭкспортаПлатежногоПоручения(Документ, ИспользуетсяСогласованиеПлатежей);
...
Функция ПолучитьТаблицуДокументовДляЭкспорта(ПлатежныеПоручения, ПлатежныеТребования) Экспорт
	...
	|	ПлатежноеПоручение.СчетКонтрагента.Владелец.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) КАК КонтрагентФизлицо,
...
Функция ЗагрузитьБанковскиеВыписки(РаспознанныеДанныеИзБанка,
	...
	Если Не РежимЗагрузки = "ЦифровойСчет" Тогда
