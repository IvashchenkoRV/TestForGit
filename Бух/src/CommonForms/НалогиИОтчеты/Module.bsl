...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненнаяФорма И Не ЗавершениеРаботы И Не ЗакрытьФормуНалогиИОтчеты Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура _ОК(Команда)
	...
	Если РежимБезЗакрывающихДокументов И НастройкиСистемыНалогообложения.ПлательщикНДСПрименяющийУСН
		И ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		Таймаут = 30;
		ТекстИнформация = НСтр("ru = 'Плательщик НДС на УСН должен формировать закрывающие документы (Накладная, Счет-фактура, УПД и т.д.). Для этого необходимо включить функционал ""Расчеты с покупателями и поставщиками"", в разделе Настройки - Функциональность - Расчеты.'");
