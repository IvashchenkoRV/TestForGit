...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СвойстваБанковскогоСчета.СчетОрганизации Тогда
		...
		ОбновитьСведенияОСервисеФинУслуги();
...
Процедура БанкПриИзменении(Элемент)
	...
	Если СвойстваБанковскогоСчета.СчетОрганизации Тогда
		...
		ФинУслугиКлиент.ОбновитьОтображениеФинУслуги(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОписаниеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	НазваниеФормы  = СтрЗаменить(ИмяФормы, ".Форма.ФормаЭлемента", "");
	ФинУслугиКлиент.ПерейтиПоСсылке(НавигационнаяСсылкаФорматированнойСтроки, ТаблицаПредложенийБанков, ТаблицаСтатистики, НазваниеФормы);
...
Процедура ПоказатьЭлементыНастроекПодключения(Форма, ТекущийТекстНомераСчета = "")
	...
	Если РазрешенаНастройкаОбмена И Не Форма.БанкВРежимеИнтеграции Тогда
		ВидимостьГиперссылкиДиректБанк = Форма.ВидимостьЭлементовДиректБанк = "ПоказатьГиперссылку"
			Или Форма.ВидимостьЭлементовДиректБанк = "ПоказатьБаннер";
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьСведенияОСервисеФинУслуги()
...
Процедура Подключаемый_ОтправитьСтатистику()
	...
	ДлительнаяОперацияСуществует = ЗначениеЗаполнено(СведенияОДлительнойОперации)
