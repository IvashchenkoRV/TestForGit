...
Процедура СменитьСтраницуШаги(НоваяСтраница)
	...
	ИначеЕсли НоваяСтраница = Элементы.СтраницаSberAPI Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Для настройки перейдите в личный кабинет банка и выполните действия согласно '")));
...
&НаКлиенте
Процедура ОтобразитьСекретныеДанные(Элемент, ИмяРеквизита) Экспорт
	
	Элемент.РежимПароля = Не Элемент.РежимПароля;
	ЭтотОбъект[ИмяРеквизита] = Элемент.ТекстРедактирования;
...
Процедура ПослеПолученияСертификатовОрганизацииИДанныхПользователяSberAPI(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		Если РезультатЗадания.ИнформацияОПользователе.ВидПодписи <> "Единственная подпись" Тогда
			...
			ТекстСообщения = НСтр("ru = 'Обратите внимание, что при текущей настройке ДиректБанк'") + Символы.ПС + 
				НСтр("ru = 'подписывать документы сможете только вы.'") + Символы.ПС + 
				НСтр("ru = 'Чтобы такая возможность была у других пользователей,'") + Символы.ПС +
				НСтр("ru = 'настроить обмен должен пользователь с полными правами в Сбербанке.'");
...
Процедура ЗагрузитьФайлНастроекSberAPIЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		// Считаем все необходымые поля из файла JSON
	...
	Если РеквизитыОрганизации.ИНН <> НастройкиИзФайла.ОрганизацияИНН Тогда
	...
	Если СертификатПодключенияДатаОкончания <= НачалоДня(ТекущаяДата()) Тогда
