...
Функция ДокументМожетБытьОформленВЦифровыхРублях(ВидОперации, Период) Экспорт
	
	Если Не ЦифровойРубль.ДоступенПлатежОбменСЦифровымКошельком(Период) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Результат = ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет;
