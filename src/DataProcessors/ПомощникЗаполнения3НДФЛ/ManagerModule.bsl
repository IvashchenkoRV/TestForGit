...
Функция СоответствиеДоходовИсточникам(ВыбраннаяФорма) Экспорт
	...
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма) Тогда
		...
		Результат.Вставить(Перечисления.ИсточникиДоходовФизическихЛиц.ЦифровойАктив, "009");
		...
		ДобавитьСоответствиеКодовДоходов(Результат, КодыДоходовЦенныеБумаги.Облигации, "014");
...
Функция РассчитатьНалог(Организация, Период, ДанныеПомощникаЗаполнения, ВыбраннаяФорма3НДФЛ)
	...
	Для Каждого СтрокаТаблицы Из ДоходыВычеты Цикл
		...
		ИначеЕсли СтрокаТаблицы.Вид = Перечисления.ИсточникиДоходовФизическихЛиц.ПродажаИмущества
			...
			Если СтрокаТаблицы.СтруктураДоходовВычетов.ВидИмущества = ПомощникЗаполнения3НДФЛКлиентСервер.ВидДоходаДоляУставногоКапитала() Тогда
				НоваяСтрока.ВидДохода = СтрокаТаблицы.СтруктураДоходовВычетов.ДанныеОтчетности.КодВидаДохода;
	...
	ТаблицаНалоговойБазы = ТаблицаНалоговойБазы(ТаблицаДанныхЗаполнения, ВыбраннаяФорма3НДФЛ, Период);
	...
	Для Каждого СтрокаНалоговойБазы Из ТаблицаНалоговойБазы Цикл
		...
		Если СтрокаНалоговойБазы.ВидДохода = ВидДоходаПоУмолчанию Тогда
...
Процедура ДобавитьРасчетНалоговойБазыПоВидуДохода(Вид, ДоходВычет, РасчетНалоговойБазы, ВыбраннаяФорма)
	...
	ИначеЕсли Вид = Перечисления.ИсточникиДоходовФизическихЛиц.ОперацииСЦеннымиБумагами Тогда
		...
		Если РасчетНалоговойБазы.ОбщаяСуммаДоходов = 0 И ЭтоДоходПоВкладам(ДоходВычет, ВыбраннаяФорма) Тогда
...
Функция ЭтоДоходПоВкладам(СтрокаДохода, ВыбраннаяФорма)
...
Функция ТаблицаДанныхРасчетаНалоговыхБаз(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма)
	...
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма) Тогда
		Результат = ТаблицаДанныхРасчетаНалоговыхБаз2025(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма);
	Иначе
		Результат = ТаблицаДанныхРасчетаНалоговыхБаз2022(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма);
	КонецЕсли;
...
Процедура ДобавитьИтоговыеПоказателиПоОперациямСЦеннымиБумагами2022(ТаблицаДанныхЗаполнения, ВыбраннаяФорма3НДФЛ)
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		НеоблагаемыйДоход = ПомощникЗаполнения3НДФЛ.НеоблагаемыйДоходПоВкладам(ВыбраннаяФорма3НДФЛ) * КлючеваяСтавкаЦБ(ВыбраннаяФорма3НДФЛ) / 100;
...
Функция ТаблицаДанныхРасчетаНалоговыхБаз2022(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма)
...
Процедура РассчитатьПоказателиНалоговойБазыПоПрогрессивнойШкале(РасчетНалоговойБазы, СуммаНакопительнымИтогом, Период)
...
Процедура ЗаполнитьПоясненияРасчетаНалогаВДекларации2022(ПараметрыВыполнения)
	...
	Для Каждого СтрокаНалоговойБазы Из ТаблицаНалоговойБазы Цикл
		...
		Если СтрокаНалоговойБазы.ВидДохода = ВидДоходаВклады Тогда
			...
			Показатели.НеоблагаемыйДоход = ПомощникЗаполнения3НДФЛ.НеоблагаемыйДоходПоВкладам(ВыбраннаяФорма3НДФЛ) * Показатели.КлючеваяСтавка / 100;
	...
	Для Каждого ТекущаяСтрока Из СовокупностьНалоговыхБаз Цикл
		...
		ДобавитьПояснениеРасчета(НоваяСтрока, ТекущаяСтрока.Значение, НалоговаяСтавкаСтрока, ПараметрыВыполнения.Период);
	...
	Если СовокупностьНалоговыхБаз.Количество() = 0 Тогда
		...
		ДобавитьПояснениеРасчета(НоваяСтрока, Показатели, Строка(СтавкаПоУмолчанию), ПараметрыВыполнения.Период);
	КонецЕсли;
...
Процедура ДобавитьПояснениеРасчета(Результат, ПоказателиРасчета, НалоговаяСтавкаСтрока, Период)
	...
	Если ЭтоИдентификаторСтавкиПрогрессивнойШкалы(НалоговаяСтавкаСтрока) И ПараметрыРасчета.БазовыйНалог > 0 Тогда
...
Процедура РассчитатьСуммуДоходовНеПодлежащуюНалогообложению_2022(ТаблицаДанныхЗаполнения, НеоблагаемыеСуммыДоходов, ВыбраннаяФорма)
	...
	Если ТаблицаДанныхЗаполнения.НайтиСтроки(ОтборВклады).Количество() > 0 Тогда
		НеоблагаемыйДоходВклады = ПомощникЗаполнения3НДФЛ.НеоблагаемыйДоходПоВкладам(ВыбраннаяФорма) * КлючеваяСтавкаЦБ(ВыбраннаяФорма) / 100;
	КонецЕсли;
...
Функция ТаблицаДанныхРасчетаНалоговыхБаз2025(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма)
	...
	Для Каждого ТекущаяСтрока Из ДоходыВычеты Цикл
		...
		ТекущаяСтрока.ГруппаДоходов = ПомощникЗаполнения3НДФЛ.КодГруппыДохода(ТекущаяСтрока.ВидДохода, ВыбраннаяФорма);
		Если ТипЗнч(ТекущаяСтрока.Вид) = Тип("ПеречислениеСсылка.ИсточникиДоходовФизическихЛиц") Тогда
...
Функция ТаблицаНалоговойБазы2025(ТаблицаДанныхЗаполнения, ПределыВычетов, ВыбраннаяФорма, Период)
	...
	ДоходыВычеты = ТаблицаДанныхРасчетаНалоговыхБаз(ТаблицаДанныхЗаполнения, НалоговыеСтавки, ВыбраннаяФорма);
	...
	Для ИндексСтроки = 0 По КоличествоСтрок - 1 Цикл
		...
		Для Индекс = ИндексСтроки + 1 По КоличествоСтрок - 1 Цикл
			...
			Если СтрокаТаблицы.ВидДохода = ""
				...
				// Для одного вида дохода может быть несколько источников
				Для Каждого ТекущееРаспределениеВычета Из РаспределениеВычета Цикл
					Если Не ПрименитьВычетПоДоходу(ТекущаяСтрока, ТекущееРаспределениеВычета.Ключ, ВыбраннаяФорма) Тогда
						Продолжить;
					КонецЕсли;
		...
		РассчитатьИтогиРасчетаНалоговойБазы(РасчетНалоговойБазы, ДоходыВычеты, ПределыВычетов, ВыбраннаяФорма);
	...
	Если ТаблицаНалоговойБазы.Количество() = 0 Тогда
		...
		НоваяСтрока = ТаблицаДанныхЗаполнения.Добавить();
		НоваяСтрока.ВидДохода = КодВидаДохода(ВыбраннаяФорма);
		НоваяСтрока.НалоговаяСтавка = СтавкаНалога(ВыбраннаяФорма);
		НоваяСтрока.СтруктураДанных = НоваяСтруктураРасчетаНалоговойБазы(
			НоваяСтрока.НалоговаяСтавка,
			НоваяСтрока.ВидДохода);
...
Функция ТаблицаСведенийОСуммахНалога2025(ПараметрыВыполнения)
	...
	ВидДоходаВклады = КодВидаДохода(ВыбраннаяФорма, "БанковскиеПроценты");
	ВидыДоходаФиксПрибыльКИК = КодВидаДохода(ВыбраннаяФорма, "ФиксированнаяПрибыльКИК");
	ВидДоходаДивиденды = КодВидаДохода(ВыбраннаяФорма, "Дивиденды");
	ВидДоходаПоУмолчанию = КодВидаДохода(ВыбраннаяФорма);
	
	ВидыДоходаПрибыльКИК = Новый Массив;
	ВидыДоходаПрибыльКИК.Добавить(КодВидаДохода(ВыбраннаяФорма, "ПрибыльКИК"));
	ВидыДоходаПрибыльКИК.Добавить(КодВидаДохода(ВыбраннаяФорма, "РаспределениеПрибылиИностраннойСтруктуры"));
	...
	Если ТаблицаРасчетов.Количество() = 0 Тогда
		СтрокаРасчета = ТаблицаРасчетов.Добавить();
		СтрокаРасчета.КБК = ПомощникЗаполнения3НДФЛ.КБКПоВидуДохода(Перечисления.ВидыНалогов.НДФЛ_ИП, ПараметрыВыполнения.Период);
		СтрокаРасчета.ОКТМО = ОКТМОналогоплательщика;
	Иначе
		ТаблицаРасчетов.Сортировать("КБК, ОКТМО");
	КонецЕсли;
...
Процедура ДобавитьДанныеЗаявленияНаВозвратНалога2025(ТаблицаДанныхЗаполнения, СчетОрганизации, ВыбраннаяФорма3НДФЛ)
	...
	Для Каждого СтрокаСведений Из ТаблицаДанныхЗаполнения Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураДанных, "СуммаНалогаПодлежащаяВозврату", 0) > 0 Тогда
			СуммаКВозврату = СуммаКВозврату + СтруктураДанных.СуммаНалогаПодлежащаяВозврату;
		Иначе
			СуммаКВозврату = СуммаКВозврату
				+ Макс(СтрокаСведений.СуммаНалогаУплаченнаяУдержанная
				- СтрокаСведений.СуммаНалогаИсчисленная, 0);
		КонецЕсли;
...
Процедура ДобавитьДанныеЗаполненияИмущественногоВычета(ТаблицаДанныхЗаполнения, ДанныеФормы, ВидДохода, НалоговаяСтавка, ТаблицаПоказателей, КодыИсточниковДоходов)
	...
	Если ДанныеФормы.ВидИмущества <> ПомощникЗаполнения3НДФЛКлиентСервер.ВидДоходаДоляУставногоКапитала() Тогда
