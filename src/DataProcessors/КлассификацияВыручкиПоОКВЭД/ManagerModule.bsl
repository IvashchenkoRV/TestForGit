...
Функция ДанныеРаспределенияВыручки(Параметры) Экспорт
	...
	ПервоначальноеЗаполнениеОКВЭДомИзОрганизации = Ложь;
	Если Не ЗначениеЗаполнено(ДанныеНоменклатуры) Или Параметры.ПерезаполнитьДанныеНоменклатуры Тогда
		...
		ПолучитьДанные(ДанныеНоменклатуры, НастройкиОКВЭД, Параметры.Организация, Параметры.ДатаНачала, Параметры.ДатаОкончания, Параметры.ИмяОтчета);
	КонецЕсли;
	...
	Если ПервоначальноеЗаполнениеОКВЭДомИзОрганизации Тогда
		ЗаполнитьОКВЭДомИзОрганизации(ПоляГруппировок, ДанныеНоменклатуры, СуммаВыручки, ОКВЭДОрганизации, СоответствиеЦветов);
	КонецЕсли;
...
Функция ОбновитьДанныеРаспределенияВыручки(Параметры) Экспорт
	...
	ОбновитьДанныеНоменклатуры(ОКВЭД, ПоляПоискаГруппировкиНоменклатуры, ДанныеНоменклатуры, ПоляГруппировок, СоответствиеЦветов);
	СуммаВыручки = ДанныеНоменклатуры.Итог("СуммаВыручки");
...
Процедура ПолучитьДанные(ДанныеНоменклатуры, НастройкиОКВЭД, Организация, ДатаНачала, ДатаОкончания, ИмяОтчета)
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		ЗаполнитьОКВЭДИзНастройки(НовСтрока, НастройкиОКВЭД, НастройкиОКВЭДПоГруппамИ);
...
Процедура ЗаполнитьОКВЭДИзНастройки(Строка, НастройкиОКВЭД, НастройкиОКВЭДПоГруппамИ)
...
Процедура ЗаполнитьОКВЭДомИзОрганизации(ПоляГруппировок, ДанныеНоменклатуры, СуммаВыручки, ОКВЭДОрганизации, СоответствиеЦветов)
	
	ДанныеПоПолюГруппировок = ДанныеНоменклатуры.Скопировать();
	Если ПоляГруппировок.Количество() = 1 Тогда
		КолонкиГруппировок = СтрШаблон("%1", ПоляГруппировок[0].Значение);
		ИндексУровня = 0;
	Иначе
	...
	Для Каждого СтрокаДанных Из ДанныеПоПолюГруппировок Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.ОКВЭД) И ТекСуммаВыручки <= СуммаВыручки Тогда
			ПоляПоискаГруппировкиНоменклатуры = Новый Массив;
			ЭлементПолейПоиска = Новый Структура();
			ЭлементПолейПоиска.Вставить("ПолеГруппировкиНоменклатуры", СтрокаДанных[ПоляГруппировок[ИндексУровня].Значение]);
			...
			ОбновитьДанныеНоменклатуры(ОКВЭДОрганизации, ПоляПоискаГруппировкиНоменклатуры, ДанныеНоменклатуры, ПоляГруппировок, СоответствиеЦветов);
...
Процедура ОбновитьДанныеНоменклатуры(ОКВЭД, ПоляПоискаГруппировкиНоменклатуры, ДанныеНоменклатуры, ПоляГруппировок, СоответствиеЦветов)
