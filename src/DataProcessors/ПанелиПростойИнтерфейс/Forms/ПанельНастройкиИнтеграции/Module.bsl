...
Процедура ОбменСБанками(Команда)
	...
	ПараметрыФормы.Вставить("Организация", ОбщегоНазначенияБПВызовСервера.ОсновнаяОрганизация());
	
	ОткрытьФорму("Справочник.НастройкиОбменСБанками.Форма.ФормаСписка", ПараметрыФормы, ЭтотОбъект);
...
&НаКлиенте
Процедура ИнтеграцияBnovo(Команда)
	ПараметрыФормы = Новый Структура("ПоказыватьКомандыЗагрузки", Ложь);
	ОткрытьФорму("РегистрСведений.НастройкиИнтеграцииBnovo.ФормаСписка", ПараметрыФормы);
КонецПроцедуры
