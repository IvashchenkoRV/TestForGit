...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода) Тогда
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
			КомпоновщикНастроек, "НачалоПериода", НачалоКвартала(ПараметрыОтчета.КонецПериода));
	КонецЕсли;
...
Процедура ПриВыводеЗаголовка(Контекст, КомпоновщикНастроек, Результат) Экспорт
	...
	Контекст.НачалоПериода = НачалоКвартала(Контекст.КонецПериода);
	Контекст.ПредставлениеПериода = ПредставлениеПериода(
		Контекст.НачалоПериода, КонецКвартала(Контекст.КонецПериода), "ДФ='MMMM yyyy ""г.""'");
...
Функция ПолучитьПараметрыИсполненияОтчета() Экспорт
    ...
    Манифест.Вставить("ПериодичностьОтчета",               Перечисления.ДоступныеПериодыОтчета.Квартал);
