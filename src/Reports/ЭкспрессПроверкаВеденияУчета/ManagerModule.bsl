...
Функция ПроверитьСоответствие91_НДС_БУ(СтрокаПроверки, ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("СтавкаНДСПредположительная", УчетНДСКлиентСервер.ОбщаяСтавкаНДС(КонецДня(ПараметрыОтчета.КонецПериода)));
...
Функция ПроверитьСоответствие90и91_НДС_НУ(СтрокаПроверки, ПараметрыОтчета) Экспорт
	...
	|	
	|	ВЫБРАТЬ
	// В случае отчета о розничных продажах при возвратах в качестве счета-фактуры записывается ссылка
	// на предудущий отчет о розничных продажах, которым была отражена первичная продажа.
	// Но в регистре бухгалтерии регистратором будет отчет на возврат и он же является регистратором для книги покупок.
	// поэтому выбираем не счет-фактуру, а регистратор в качестве документа для сопоставления с БУ.
	...
	|	ГДЕ
	|		НДСПокупки.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах) КАК ДанныеНДС
	|
	|СГРУППИРОВАТЬ ПО
