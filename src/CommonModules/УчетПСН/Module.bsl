...
Функция ВзносыИспользованныеДляУменьшенияПатента(Организация, Период, РасчетДляРегламентнойОперации = Ложь, УплаченоВТекущейОперации = 0) Экспорт
	...
	ВзносПоЕдиномуТарифуРассчитанный = Мин(ВзносПоЕдиномуТарифуРассчитанный, ЕдиныйТарифОстаток);
	...
	ВзносСДоходаПредыдущийГодРассчитанный = Мин(ВзносСДоходаПредыдущийГодРассчитанный, СДоходаПрошлыйГодОстаток);
	...
	ВзносСДоходаТекущийГодРассчитанный = Мин(ВзносСДоходаТекущийГодРассчитанный, СДоходаТекущийГодОстаток);
...
Процедура УстановитьПорядокОтраженияАванса(ПорядокОтраженияАванса, ПараметрыОбъекта) Экспорт
	...
	Если ТипЗнч(ПорядокОтраженияАванса) = Тип("СправочникСсылка.Патенты")
		И Не ДеятельностьПоПатентуДействуетНаДату(ПорядокОтраженияАванса, ДатаДокумента) Тогда
		ПорядокОтраженияАванса = Неопределено;
	КонецЕсли;
	
	// Если патент закончился, выберем УСН
...
Процедура ПослеЗаписиДокументаРеализацииПолномочийНалоговыхОрганов(Сообщение, Файлы) Экспорт
	...
	Попытка
		...
		Если ЕстьДанныеЗаявления И НапоминанияПользователя.ИспользуютсяНапоминанияПользователя() Тогда
			...
			Если ТипЗнч(ДанныеЗаявления) = Тип("Структура") И ДанныеЗаявления.Свойство("ДанныеПомощникаЗаполнения")
				И ТипЗнч(ДанныеЗаявления.ДанныеПомощникаЗаполнения) = Тип("Структура")
				И ДанныеЗаявления.ДанныеПомощникаЗаполнения.Свойство("Ответственный")
				И ЗначениеЗаполнено(ДанныеЗаявления.ДанныеПомощникаЗаполнения.Ответственный) Тогда
				...
				ОписаниеПатента.Вставить("Ответственный", ДанныеЗаявления.ДанныеПомощникаЗаполнения.Ответственный);
