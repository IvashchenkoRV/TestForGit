...
Функция ВыручкаИСебестоимость(ПараметрыОтчета, ПрошлыйПериод = Ложь) Экспорт
	...
	|		ВЫРАЗИТЬ(СебестоимостьПроизводства.СубконтоКт3 КАК Справочник.Номенклатура),
	...
	|		ИСТИНА
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, Авто, СчетДт В (&СчетаСебестоимости), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы), СчетКт В (&СчетаУчетаПроизводства), &СубконтоНоменклатурныеГруппыСтатьиЗатратПродукция, ИСТИНА) КАК СебестоимостьПроизводства
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
