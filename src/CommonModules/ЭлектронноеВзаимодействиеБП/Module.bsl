...
Процедура ДобавитьОписаниеПрочитанногоФайла(РезультатЧтения, ДанныеДокумента, ПолноеИмяФайла)
	...
	ИначеЕсли ДанныеДокумента.ВидДокумента = ВидДокументаОтчетOzonОприходование() Тогда
		...
		Описание = СтрШаблон(НСтр("ru='Отчет о декомпенсации Ozon от %1
			|Сумма возврата компенсации: %2'"), Формат(ДанныеДокумента.Дата, "ДФ=dd.MM.yyyy"), Формат(СуммаВозвратов, "ЧДЦ=2; ЧН="));
...
Процедура ОпределитьПериодРеестраПродажЮрлицамWB(ДанныеДокумента)
	...
	Для Каждого СтрокаТовара Из ДанныеДокумента.ТаблицаТоваров Цикл 
		Если ЗначениеЗаполнено(СтрокаТовара.ДатаДокумента) Тогда
			ДанныеДокумента.Вставить("Дата", КонецНедели(СтрокаТовара.ДатаДокумента));
			Возврат;
		КонецЕсли;
	КонецЦикла;
