...
Функция НалоговыеСтавки(Период) Экспорт
	...
	Если УчетНДФЛПредпринимателяКлиентСервер.ПрименяетсяРасширеннаяПрогрессивнаяйШкала(Период) Тогда
	...
	ИначеЕсли Период >= УчетНДФЛПредпринимателяКлиентСервер.ДатаНачалаПрогрессивнойШкалы() Тогда
		// Базовая ставка налога
		НоваяСтрока = Результат.Добавить();
		НоваяСтрока.СтавкаНалога = 13;
		НоваяСтрока.МаксимальнаяСуммаДохода = 5000000;
...
//   Число
//
Функция СуммаНалога(НалоговаяБаза, Период) Экспорт
	
	НалоговыеСтавки = НалоговыеСтавки(Период);
	Результат = 0;
...
//    Число
//
Функция ПредельнаяСуммаДоходаПоБазовойСтавке(Период) Экспорт
...
Функция УплаченныйНалогЗаПериодВПростомУчетеЕНС(Организация, НачалоПериода, КонецПериода)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(РасчетНачисленияНалоговНаЕНССрезПоследних.Начислено), 0) КАК Сумма
	|ИЗ
