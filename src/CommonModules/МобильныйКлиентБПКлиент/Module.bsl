...
Процедура СохранитьПечатнуюФормуВФайлПослеПолученияФайлов(Результат, ДопПараметры) Экспорт
...
Процедура ПоделитьсяФайлами(ИменаФайлов) Экспорт
	...
	//До платформы 8.3.25 объект СредстваПередачиДанныхНаУстройстве не поддерживается
	Успешно = Ложь;
	#Если МобильныйКлиент Тогда
	Если Вычислить("СредстваПередачиДанныхНаУстройстве.ПоддерживаетсяВозможностьПоделитьсяДанными()") Тогда
		
		ОбъектыПоделиться = Новый Массив;
		
		Для каждого ПолноеИмя Из ИменаФайлов Цикл
			ПараметрыМетода = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПолноеИмя);
			Данные = Новый (Тип("ДанныеМультимедиа"), ПараметрыМетода); //До 8.3.25 отсутствует конструктор с 1 параметром
			ОбъектыПоделиться.Добавить(Данные);
		КонецЦикла;
		
		Успешно = Вычислить("СредстваПередачиДанныхНаУстройстве.ПоделитьсяДанными(ОбъектыПоделиться, ""*/*"")");
